export default (params) =>
{
	...
	for(let word of words)
	{
		...
		{
			filters.push( { $match: { tweet: { $regex: word, $options: 'i' } } });
		}

		// All of them needs to exclude
		if(params.mode == "Dont Match")
		{
			filters.push( { $match: { tweet: { $not: word, $options: 'i' } } });	
		}

		// At least one needs to exists
		if(params.mode == "Contain")
		{
			contains.push( { tweet: { $regex: word, $options: 'i' } });
		}

		// At least one needs to exclude
		if(params.mode == "Do not Contain")
		{
			contains.push( { tweet: { $not: word, $options: 'i' } } );	
		}
